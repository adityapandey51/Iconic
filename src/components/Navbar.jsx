...
import { menu,close,Group} from "../assets";
  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='flex w-full justify-between items-center max-w-7xl mx-auto'>
        ...
        {navLinks.map((Link)=>(<li
        key={Link.id} className={`${active===Link.title?"text-white":"text-secondary"} font-poppins hover:text-white text-[16px] font-medium cursor-pointer`} onClick={()=>{setActive(Link.title);setToggle(!toggle)}}>
          <a href={`#${Link.id}`}>{Link.title}</a>
        </li>))}
        </ul>
          </div>
